...
static void user_msg_handler(uint32_t msg_id, void *data, uint16_t size)
{
    switch (msg_id)
    {
        ...
        {
            ir_key_t t_key_ir = t_ir.key_get();
            platform_printf("IrkeyVal:0x%08x,state:0x%02x\n",t_key_ir.value,t_key_ir.state);
            break;
        }        
        default:
            break;
...
{
    static const bd_addr_t rand_addr = { 0xFE, 0x3E, 0x9F, 0xFD, 0x26, 0x7B };
